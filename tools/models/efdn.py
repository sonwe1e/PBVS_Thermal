...
class Cell(nn.Module):
    def __init__(self, n_feats=48, dynamic=True, deploy=False, L=None, with_13=False):
        ...
        self.att = ESA(
            n_feats, nn.Conv2d
        )  # MAB(n_feats)# ENLCA(n_feats)  #CoordAtt(n_feats,n_feats,10)#

        self.branch = nn.ModuleList(
            [nn.Conv2d(n_feats, n_feats // 2, 1, 1, 0) for _ in range(4)]
        )
...
class EFDN(nn.Module):
    def __init__(self, scale=4, in_channels=3, n_feats=48, out_channels=3):
        ...
        self.local_fuse = nn.ModuleList(
            [nn.Conv2d(n_feats * 2, n_feats, 1, 1, 0) for _ in range(3)]
        )
