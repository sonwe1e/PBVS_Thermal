...
class DAT(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module): Normalization layer. Default: nn.LayerNorm
        use_chk (bool): Whether to use checkpointing to save memory.
        upscaling_factor: Upscale factor. 2/3/4 for image SR
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.upsampler == "pixelshuffle":
            ...
            self.upsample = Upsample(upscaling_factor, num_feat)
            self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
        elif self.upsampler == "pixelshuffledirect":
            # for lightweight SR (to save parameters)
            self.upsample = UpsampleOneStep(
                upscaling_factor, embed_dim, num_out_ch, (img_size, img_size)
            )
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("checkpoints/DAT_S_x2.pth")["params"])
    print(height, width)
