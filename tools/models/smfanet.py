...
class RSMFANET(nn.Module):
    def __init__(self, dim=64, n_blocks=12, ffn_scale=2, upscaling_factor=2):
        super().__init__()
        self.scale = upscaling_factor
        self.to_feat = nn.Conv2d(3, dim, 3, 1, 1)
        self.fmb = FMB(dim, ffn_scale)  # Single FMB layer to be reused
    ...
    def forward(self, x):
        ...
        # Recursively apply the same FMB block n_blocks times
        feat = x
        for _ in range(self.n_blocks):
            feat = self.fmb(feat)

        x = feat + x  # Global residual connection
        x = self.to_img(x)
        return x
