...
def trunc_normal_(tensor, mean=0.0, std=1.0):
    # This implementation fills the tensor with values drawn from a normal distribution
    # and then truncates the values that are more than 2 standard deviations from the mean.
    # It uses a rejection sampling strategy.
    with torch.no_grad():
        size = tensor.shape
        # Extra dimension for more samples
        tmp = tensor.new_empty(size + (4,)).normal_()
        # Create a mask for samples within 2 std deviations
        valid = (tmp < 2) & (tmp > -2)
        # Find the first valid sample along the last dimension
        ind = valid.float().argmax(dim=-1)
        tensor.data.copy_(tmp.gather(-1, ind.unsqueeze(-1)).squeeze(-1))
...
if __name__ == "__main__":
    model = HAT()
    ckpt = torch.load(
        "/media/hdd/sonwe1e/Competition/PBVS_Thermal/HAT-L_SRx4_ImageNet-pretrain.pth"
    )["params_ema"]
